...
//Call.Factory  Call可用于发送HTTP请求并读取其响应

//OkHttpClients should be shared
//OkHttpClient的实例应该使用一个
...
//您可以使用{@link #newBuilder（）}自定义共享的OkHttpClient实例。 这将生成共享相同连接池，线程池和配置的客户端。 使用构建器方法为特定目的配置派生客户端。

//This example shows a call with a short 500 millisecond timeout:
//这个例子显示了一个短暂的500毫秒超时的请求：
...
//但是如果你正在编写一个需要积极释放未使用资源的应用程序，你可以这样做。
//Shutdown the dispatcher's executor service with {@link ExecutorService#shutdown shutdown()}.
//使用{@link ExecutorService＃shutdown shutdown（）}关闭调度程序的执行程序服务。
...
//如果您的客户端有缓存，请调用{@link Cache＃close close（）}。 请注意，针对已关闭的缓存创建调用是错误的，这样做会导致调用崩溃。
//client.cache().close();

//OkHttp also uses daemon threads for HTTP/2 connections. These will exit automatically if they remain idle.
//OkHttp还为HTTP / 2连接使用守护进程线程。 如果它们保持空闲，它们将自动退出。
...
public class OkHttpClient implements Cloneable, Call.Factory, WebSocket.Factory {
     ...
     * 从连接建立之前（如果有）到选择响应源（源服务器，高速缓存或两者）之后，返回观察每个调用的完整跨度的不可变拦截器列表。
     */
     ...
     * <p>
     * 返回观察单个网络请求和响应的拦截器的不可变列表。 这些拦截器必须只调用一次{@link Interceptor.Chain＃}}：对于网络拦截器来说，短路或重复发送网络请求是错误的
     */
    ...
    public static final class Builder {
        ...
        //认证 响应来自远程Web服务器或代理服务器的身份验证 返回null来拒绝盘问
        ...
        public Builder() {
            ...
            //通过 Cookie 可以让服务器知道两个请求是否来自于同一个客户端
            //从不接受任何cookie
         ...
         * for this client.
         * 配置工厂以提供每个调用范围的侦听器，这些侦听器将接收此客户端的分析事件。
         *
         * @see EventListener for semantics and restrictions on listener implementations.
         */
